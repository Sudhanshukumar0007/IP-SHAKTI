...
class ResearchTask(TypedDict):
    """A decomposed task generated by the supervisor."""
    id: str
    question: str
    domain: str
    jurisdiction: str    # "national" | "international"
...
class TaskResult(TypedDict):
    """Result of a single research task executed by the worker."""
    task_id: str
    question: str
    jurisdiction: str
    domain: str
...
class AgentState(TypedDict):
    ...
    session_id: str
    raw_query: str
    search_query: Optional[str]      # current query used for retrieval (might be rewritten)
    query_rewritten: bool            # tracks whether the query has been rewritten
    language: str                    # e.g. "en", "hi"
    jurisdiction_mode: str           # "national" | "international" | "both"

    # ── Intent detection (set by detect_intent node, 0 LLM calls) ─────────────
    # "informational" → plain lookup query → skip classification tree, go straight to retrieve
    # "classification" → product/formulation query → go through five-gate tree
    # "unknown" → ambiguous → fall through to auto_classify heuristics
    query_intent: Optional[str]      # "informational" | "classification" | "unknown"
    ...
    overall_status: Optional[str]         # "VERIFIED" | "PARTIAL" | "ABSTAIN"
    verified_tasks: list[ResearchTask]
    insufficient_tasks: list[ResearchTask]

    # ── Retrieval — jurisdiction-isolated, never merged ────────────────────────
    # These flat lists are now populated by the evidence_verification node
